...
<span style='color:#aaffaa;font-size:0.85em'>{b.get('spr_victim','Receiver susceptibility mask; antenna gain; receiver noise power')}</span>
    ...
    # Full table — grouped by Working Party
    st.subheader("All Bands Summary — Grouped by Working Party")
    ex("Bands grouped by the ITU-R Working Party responsible for their protection in WRC-27. Effective threshold = I/N threshold + aviation safety factor per M.1477.")
    ...
    # Build rows dict keyed by band name for lookup
    rows_by_band = {}
    ...
    plotted = set()
    for grp_label, band_names in WP_GROUPS.items():
        # skip if no bands in this group match (avoids duplicating Radio Altimeter)
        unique_bands = [b for b in band_names if b in rows_by_band and b not in plotted]
        if not unique_bands:
            continue

        color_key = next((k for k in GRP_COLORS if k in grp_label), "All WPs")
        hex_color = GRP_COLORS[color_key]
        st.markdown(
            f"<div style='background:{hex_color};padding:6px 12px;border-radius:4px;"
            f"margin:10px 0 4px 0;font-weight:bold;color:white'>{grp_label}</div>",
            unsafe_allow_html=True
        )
        grp_rows = [rows_by_band[b] for b in unique_bands]
        st.dataframe(pd.DataFrame(grp_rows), use_container_width=True, hide_index=True)
        for b in unique_bands:
            plotted.add(b)

    # Any bands not in a group
    ungrouped = [rows_by_band[n] for n in FAA_BANDS if n not in plotted]
    if ungrouped:
        st.markdown("**Other Protected Bands**")
        st.dataframe(pd.DataFrame(ungrouped), use_container_width=True, hide_index=True)
...
"""
    ...
    with oob_c1:
        ...
        _pwr_default = 39.0   # 39 dBW = 8 kW
        oob_power_db = st.number_input("Peak Power (dBW)", value=_pwr_default, step=0.5,
            help="Transmitter peak output power in dBW (ITU-R standard unit). "
                 "0 dBW = 1 W · 10 dBW = 10 W · 20 dBW = 100 W · 30 dBW = 1 kW · 39 dBW = 8 kW · 43 dBW = 20 kW")

        # Derive Watts for the spurious formula and display
        p_dbw = oob_power_db
        oob_power_w = 10 ** (p_dbw / 10)
        st.caption(f"= **{p_dbw+30:.1f} dBm** = **{oob_power_w:.1f} W**")
    with oob_c2:
        # OOB boundary
        half_bw      = oob_bw / 2
        ...
        # RR Appendix 3 spurious limit — uses dBW directly, no conversion needed
    ...
    # ── Live step-by-step sanity check ───────────────────────────────────────
    with st.expander("🔍 Step-by-step sanity check — show the math", expanded=True):

        # Determine which edge is closer
        dist_from_low  = abs(oob_check_freq - band_low)
        dist_from_high = abs(oob_check_freq - band_high)
        closer_edge    = band_low  if dist_from_low  < dist_from_high else band_high
        closer_label   = "lower"   if dist_from_low  < dist_from_high else "upper"
        dist_to_edge   = min(dist_from_low, dist_from_high)
        oob_limit_this_side = oob_low if closer_label == "lower" else oob_high
        pct_of_bn      = (dist_from_edge / oob_bw * 100) if dist_from_edge > 0 else 0

        st.markdown(f"""
**Step 1 — Locate the transmitter channel edges**

| Parameter | Calculation | Result |
|---|---|---|
| Center frequency | Input | **{oob_center:.1f} MHz** |
| Occupied BW (BN) | Input | **{oob_bw:.1f} MHz** |
| Lower band edge | {oob_center:.1f} − {oob_bw/2:.1f} | **{band_low:.1f} MHz** |
| Upper band edge | {oob_center:.1f} + {oob_bw/2:.1f} | **{band_high:.1f} MHz** |

---

**Step 2 — Calculate the OOB domain boundaries (SM.1540: 250% of BN each side)**

| Parameter | Calculation | Result |
|---|---|---|
| OOB zone width each side | 250% × {oob_bw:.1f} MHz | **{oob_bw*2.5:.1f} MHz** |
| OOB lower boundary | {band_low:.1f} − {oob_bw*2.5:.1f} | **{oob_low:.1f} MHz** |
| OOB upper boundary | {band_high:.1f} + {oob_bw*2.5:.1f} | **{oob_high:.1f} MHz** |
| Beyond boundaries → | Spurious domain | RR Appendix 3 applies |

---

**Step 3 — Classify {oob_check_freq:.1f} MHz**

| Parameter | Calculation | Result |
|---|---|---|
| Distance from {closer_label} band edge | abs({oob_check_freq:.1f} - {closer_edge:.1f}) | **{dist_to_edge:.1f} MHz** |
| Expressed as % of BN | {dist_to_edge:.1f} ÷ {oob_bw:.1f} × 100 | **{dist_to_edge/oob_bw*100:.0f}% of BN** |
| OOB zone limit ({closer_label} side) | {oob_bw*2.5:.1f} MHz from edge | **{oob_limit_this_side:.1f} MHz** |
| In OOB zone? | {dist_to_edge:.1f} MHz vs {oob_bw*2.5:.1f} MHz limit | **{'YES' if dist_from_edge > 0 and dist_from_edge <= oob_boundary else ('NO — spurious domain' if dist_from_edge > oob_boundary else 'IN-CHANNEL')}** |
""")

        # Step 4 — applicable rule
        if dist_from_edge <= 0:
            st.markdown(f"""
---
**Step 4 — Applicable rule: IN-CHANNEL**

{oob_check_freq:.1f} MHz is inside the transmit channel ({band_low:.1f}–{band_high:.1f} MHz).
This is **in-band** interference — the victim receives co-channel emissions at full carrier power.
No OOB or spurious mask applies; the co-channel I/N criterion governs instead.
""")
        elif dist_from_edge <= oob_boundary:
            st.markdown(f"""
---
**Step 4 — Applicable rule: SM.1541 OOB Mask**

{oob_check_freq:.1f} MHz is **{dist_from_edge:.1f} MHz** beyond the {closer_label} band edge — that is **{pct_of_bn:.0f}% into the OOB zone** (which extends to {oob_bw*2.5:.0f} MHz = 250% of BN).

SM.1541 Table 1 requires the emission to be attenuated by **≥23 dB** at the band edge ({closer_edge:.1f} MHz) relative to the mean carrier power. The mask then continues rolling off further into the OOB zone.

Because {oob_check_freq:.1f} MHz has **not crossed** the OOB/spurious boundary at {oob_limit_this_side:.1f} MHz, the milder OOB mask applies — not the absolute RR Appendix 3 spurious floor.

> **In a contribution challenge, cite:** *"The frequency {oob_check_freq:.1f} MHz falls within the OOB domain ({dist_from_edge:.1f} MHz from the {closer_label} band edge, {pct_of_bn:.0f}% of BN). Per ITU-R SM.1541, the proponent must demonstrate OOB emissions are attenuated by 23 dB or more at {closer_edge:.1f} MHz."*
""")
        else:
            pct_beyond = (dist_from_edge / (oob_bw * 2.5)) * 100
            # SM.1541 absolute OOB mask level at this distance (simplified linear ramp)
            sm1541_mask = min(23 + (dist_from_edge - oob_bw * 2.5) / (oob_bw * 2.5) * 17, 50)
            st.markdown(f"""
---
**Step 4 — Applicable rule: RR Appendix 3 Spurious Limit**

{oob_check_freq:.1f} MHz is **{dist_from_edge:.1f} MHz** beyond the {closer_label} band edge, which exceeds the 250% BN OOB boundary at {oob_limit_this_side:.1f} MHz. This places it in the **spurious emission domain**.

| Limit type | Formula | Value |
|---|---|---|
| Relative spurious limit | min(43 + 10·log₁₀({oob_power_w:.1f} W), 60) | **{spurious_rel:.1f} dBc** |
| Absolute spurious limit | {p_dbw:.1f} dBW − {spurious_rel:.1f} dBc | **{spurious_abs:.1f} dBW** |

> **In a contribution challenge, cite:** *"The frequency {oob_check_freq:.1f} MHz falls in the spurious emission domain ({dist_from_edge:.1f} MHz from the {closer_label} band edge, beyond the 250% BN OOB boundary at {oob_limit_this_side:.1f} MHz). Per RR Appendix 3, spurious emissions must not exceed {spurious_rel:.1f} dBc ({spurious_abs:.1f} dBW) for this transmitter power level."*
""")
...
# ─────────────────────────────────────────────────────────────────────────────
    ...
    # ── WP selector ───────────────────────────────────────────────────────────
    wpc_wp = st.selectbox("Working Party", [
        "WP 5D — IMT / ARNS (M.1642, P.452, P.528)",
        "WP 5B — Maritime / Radioloc. (M.1849, P.452, P.528)",
        "WP 4C — MSS Satellite (P.619, S.1586, epfd)",
        "WP 7B — Space Research / Lunar (P.619, novel geometry)",
        "WP 7C — EESS Passive (allocation only — no I/N)",
    ], key="wpc_wp_sel")

    st.markdown("---")

    # ── FAA Victim Band selector ──────────────────────────────────────────────
    wpc_band_name = st.selectbox("FAA Victim Band", list(FAA_BANDS.keys()), key="wpc_band")
    wb = FAA_BANDS[wpc_band_name]

    b1, b2, b3, b4, b5 = st.columns(5)
    b1.metric("Band Low",   f"{wb['f_low_mhz']} MHz")
    b2.metric("Band High",  f"{wb['f_high_mhz']} MHz")
    b3.metric("BW",         f"{wb['f_high_mhz']-wb['f_low_mhz']:.1f} MHz")
    b4.metric("I/N Thresh", f"{wb['in_threshold_db']} dB")
    b5.metric("Safety ΔdB", f"+{wb.get('aviation_safety_factor_db',0)} dB")

    eff_thresh = wb.get("effective_threshold_db", wb["in_threshold_db"])
    st.caption(f"**Effective protection threshold:** {eff_thresh} dB &nbsp;|&nbsp; "
               f"Noise floor: {wb['noise_floor_dbm']} dBm &nbsp;|&nbsp; "
               f"Max interference allowed: {wb['noise_floor_dbm'] + eff_thresh:.1f} dBm &nbsp;|&nbsp; "
               f"RR 1.59 safety service: {'✅ Yes' if wb.get('rr_1_59') else 'No'}")

    st.markdown("---")

    # ══════════════════════════════════════════════════════════════════════════
    # WP 5D / 5B — Terrestrial path (P.452 / P.528 / FSPL)
    # ══════════════════════════════════════════════════════════════════════════
    if wpc_wp.startswith("WP 5D") or wpc_wp.startswith("WP 5B"):
        wp_label = "WP 5D (M.1642)" if wpc_wp.startswith("WP 5D") else "WP 5B (M.1849)"
        st.subheader(f"📐 {wp_label} Interference Calculator")
        ex("Implements the Friis chain: EIRP − path loss + Gr → received interference → I/N → protection margin. "
           "All steps shown explicitly.")

        ca, cb = st.columns(2)

        with ca:
            st.markdown("**Interferer Parameters**")
            wpc_pt   = st.number_input("Tx Power (dBm)", value=46.0, step=1.0,
                help="Base station or ship transmitter RF output power.")
            wpc_gt   = st.number_input("Tx Antenna Gain (dBi)", value=15.0, step=0.5,
                help="Gain toward victim. 0 dBi = isotropic worst-case.")
            wpc_lc   = st.number_input("Tx Losses (dB)", value=2.0, step=0.5,
                help="Cable + connector + filter losses on Tx side.")
            wpc_eirp = wpc_pt + wpc_gt - wpc_lc
            st.metric("EIRP", f"{wpc_eirp:.1f} dBm = {10**(wpc_eirp/10)/1000:.2f} W eq.")

            st.markdown("**OOB / Spurious Attenuation** *(if not in-band)*")
            wpc_oob_att = st.number_input("OOB / Emission Attenuation (dB)", value=0.0, step=1.0,
                help="Additional attenuation from the SM.1541 OOB mask or RR App.3 spurious limit "
                     "if the interfering emission is not co-channel. 0 = co-channel (worst case).")
            eirp_eff = wpc_eirp - wpc_oob_att
            if wpc_oob_att > 0:
                st.caption(f"Effective emission EIRP after mask: **{eirp_eff:.1f} dBm**")

        with cb:
            st.markdown("**Propagation & Geometry**")
            wpc_freq = st.number_input("Frequency (MHz)", value=float(wb['f_low_mhz']), step=10.0,
                help="Center frequency of the interfering emission.")
            wpc_dist = st.number_input("Distance (km)", value=5.0, step=0.5, min_value=0.1)

            prop_model = st.selectbox("Propagation Model", [
                "FSPL (Free Space — worst case)",
                "P.452 (Terrestrial — ground victim)",
                "P.528 (Aeronautical — airborne victim)",
            ], key="wpc_prop")

            if "P.452" in prop_model:
                wpc_terrain = st.selectbox("Terrain / Environment",
                    ["Urban", "Suburban", "Open/Rural", "Sea surface"], key="wpc_terrain")
                terrain_extra = {"Urban":15,"Suburban":8,"Open/Rural":0,"Sea surface":-2}[wpc_terrain]
            else:
                terrain_extra = 0

            wpc_rx_gain = st.number_input("Rx Antenna Gain (dBi)", value=0.0, step=0.5,
                help="Victim receiver antenna gain toward interferer. 0 dBi = worst case.")
            wpc_rx_loss = st.number_input("Rx Losses (dB)", value=0.0, step=0.5,
                help="Rx cable, filter, LNA entry losses.")

            # Airborne bonus — aircraft at altitude receive stronger signals
            if "P.528" in prop_model:
                wpc_alt_bonus = st.number_input("Altitude Gain Bonus (dB)", value=6.0, step=1.0,
                    help="Aircraft at altitude see stronger signals than ground receivers "
                         "due to reduced terrain shielding. M.1642 typically uses +6 dB.")
            else:
                wpc_alt_bonus = 0.0

        # ── Calculations ────────────────────────────────────────────────────
        # Path loss
        fspl = 20*np.log10(wpc_dist) + 20*np.log10(wpc_freq) + 32.44
        if "P.452" in prop_model:
            path_loss = fspl + terrain_extra
        elif "P.528" in prop_model:
            # P.528 aeronautical: FSPL + atmospheric correction − altitude bonus
            path_loss = fspl - wpc_alt_bonus
        else:
            path_loss = fspl  # FSPL

        # Received interference
        rx_interf = eirp_eff - path_loss + wpc_rx_gain - wpc_rx_loss

        # Noise floor: use band value, allow override
        st.markdown("**Receiver Noise Floor**")
        nc1, nc2, nc3 = st.columns(3)
        with nc1:
            wpc_bw_mhz = st.number_input("Rx Bandwidth (MHz)",
                value=float(wb['f_high_mhz']-wb['f_low_mhz']), step=1.0, min_value=0.001)
        with nc2:
            wpc_nf = st.number_input("Noise Figure (dB)", value=5.0, step=0.5,
                help="Receiver system noise figure. Typical: 3–8 dB for aviation receivers.")
        with nc3:
            wpc_temp = st.number_input("System Temp (K)", value=290.0, step=10.0,
                help="Reference temperature. Standard = 290 K per ITU-R.")

        kTB_dbm = -174 + 10*np.log10(wpc_bw_mhz*1e6) + wpc_nf
        st.caption(f"Noise floor = −174 + 10·log({wpc_bw_mhz:.3g} MHz) + {wpc_nf} dB NF = **{kTB_dbm:.1f} dBm**")

        i_n = rx_interf - kTB_dbm
        margin = eff_thresh - i_n
        ...
        rc1.metric("Path Loss", f"{path_loss:.1f} dB",
            help=f"{'FSPL' if 'FSPL' in prop_model else prop_model}: d={wpc_dist}km, f={wpc_freq}MHz"
                 + (f", +{terrain_extra}dB terrain" if terrain_extra else ""))
        rc2.metric("Received Interference", f"{rx_interf:.1f} dBm",
            help="EIRP_eff − path_loss + Gr − Rx_losses")
        rc3.metric("I/N Ratio", f"{i_n:.1f} dB",
            help=f"I({rx_interf:.1f}) − N({kTB_dbm:.1f})")

        margin_color = "normal" if margin >= 0 else "inverse"
        rc4.metric("Protection Margin",
            f"{margin:+.1f} dB",
            delta=f"{'PROTECTED' if margin >= 0 else 'VIOLATED'}",
            delta_color=margin_color)

        if margin >= 0:
            st.success(f"✅ **PROTECTED** — Protection margin: **{margin:+.1f} dB** above effective threshold "
                       f"({eff_thresh} dB). I/N = {i_n:.1f} dB.")
        else:
            st.error(f"❌ **THRESHOLD VIOLATED** — Margin: **{margin:+.1f} dB** below effective threshold "
                     f"({eff_thresh} dB). Cite harmful interference under RR No. 4.10.")

        # Coordination zone
        st.markdown("**Coordination Zone (minimum safe separation)**")
        # Solve: EIRP_eff − (20·log(d) + 20·log(f) + 32.44) + Gr − RxL = noise + eff_thresh
        # 20·log(d) = EIRP_eff + Gr − RxL − noise − eff_thresh − 20·log(f) − 32.44 − terrain_extra
        rhs = eirp_eff + wpc_rx_gain - wpc_rx_loss - kTB_dbm - eff_thresh - 20*np.log10(wpc_freq) - 32.44 - terrain_extra
        if "P.528" in prop_model:
            rhs += wpc_alt_bonus
        if rhs > 0:
            d_coord = 10**(rhs/20)
            st.info(f"**Minimum coordination distance: {d_coord:.1f} km** "
                    f"(distance at which I/N = effective threshold of {eff_thresh} dB). "
                    f"Cite this as the exclusion zone in your contribution.")
        else:
            st.info("Even at 0.1 km the threshold is not violated — no coordination zone required at this EIRP.")

        # Full link budget breakdown
        with st.expander("📋 Full Link Budget Breakdown"):
            st.markdown(f"""
| Step | Value | Formula |
|---|---|---|
| Tx Power | {wpc_pt:.1f} dBm | Input |
| Tx Antenna Gain | +{wpc_gt:.1f} dBi | Input |
| Tx Losses | −{wpc_lc:.1f} dB | Cable + connectors |
| **EIRP** | **{wpc_eirp:.1f} dBm** | Pt + Gt − Ltx |
| OOB Attenuation | −{wpc_oob_att:.1f} dB | SM.1541 / RR App.3 |
| **Effective EIRP** | **{eirp_eff:.1f} dBm** | EIRP − mask |
| FSPL | −{fspl:.1f} dB | 20·log({wpc_dist}) + 20·log({wpc_freq:.0f}) + 32.44 |
| Terrain Correction | {'−' if terrain_extra>0 else '+'}{abs(terrain_extra):.1f} dB | {wpc_terrain if 'P.452' in prop_model else 'N/A'} |
| Alt. Bonus | +{wpc_alt_bonus:.1f} dB | Airborne geometry |
| **Total Path Loss** | **{path_loss:.1f} dB** | |
| Rx Antenna Gain | +{wpc_rx_gain:.1f} dBi | Input |
| Rx Losses | −{wpc_rx_loss:.1f} dB | Input |
| **Received Interference** | **{rx_interf:.1f} dBm** | |
| Noise Floor | {kTB_dbm:.1f} dBm | −174 + 10·log(BW) + NF |
| **I/N** | **{i_n:.1f} dB** | Pr − N |
| Effective Threshold | {eff_thresh} dB | I/N thresh + safety factor |
| **Protection Margin** | **{margin:+.1f} dB** | Threshold − I/N |
""")

        # SM.1541 OOB check
        st.markdown("---")
        st.subheader("📋 SM.1541 OOB Domain Check")
        wpc_bw_tx = st.number_input("Interferer Signal Bandwidth (MHz)", value=20.0, step=5.0,
            key="wpc_bw_tx", help="Occupied bandwidth of the interfering signal.")
        wpc_f_center = st.number_input("Interferer Center Frequency (MHz)",
            value=wpc_freq, step=10.0, key="wpc_fcenter")
        f_lower_edge = wpc_f_center - wpc_bw_tx/2
        f_upper_edge = wpc_f_center + wpc_bw_tx/2
        oob_lower = f_lower_edge - 1.5 * wpc_bw_tx   # 250% BN = 1.5×BW each side
        oob_upper = f_upper_edge + 1.5 * wpc_bw_tx
        band_in_oob = (wb['f_low_mhz'] < oob_upper) and (wb['f_high_mhz'] > oob_lower)
        band_in_spurious = not band_in_oob and (wb['f_high_mhz'] > 0)

        st.markdown(f"""
| Parameter | Value |
|---|---|
| Interferer band edge (lower) | {f_lower_edge:.1f} MHz |
| Interferer band edge (upper) | {f_upper_edge:.1f} MHz |
| OOB domain lower boundary (250% BN) | {oob_lower:.1f} MHz |
| OOB domain upper boundary (250% BN) | {oob_upper:.1f} MHz |
| FAA victim band | {wb['f_low_mhz']}–{wb['f_high_mhz']} MHz |
| FAA band in OOB domain? | {'✅ YES — SM.1541 applies (23 dB mask at boundary)' if band_in_oob else '—'} |
| FAA band in spurious domain? | {'⚠️ YES — RR Appendix 3 applies' if band_in_spurious else '—'} |
""")

    # ══════════════════════════════════════════════════════════════════════════
    # WP 4C — Satellite (P.619, epfd, ΔT/T)
    # ══════════════════════════════════════════════════════════════════════════
    elif wpc_wp.startswith("WP 4C"):
        st.subheader("🛰️ WP 4C Satellite Interference Calculator (P.619 / S.1586)")
        ex("WP 4C uses epfd (equivalent power flux density) for constellation aggregate interference, "
           "and ΔT/T for AMS(R)S receivers. Single-satellite pfd is not sufficient — S.1586 requires the full constellation aggregate.")

        sa, sb = st.columns(2)
        with sa:
            st.markdown("**Satellite Transmitter**")
            sat_pt   = st.number_input("Satellite Tx Power (dBW)", value=10.0, step=1.0,
                help="Single satellite RF output in dBW (not dBm).")
            sat_gt   = st.number_input("Satellite Antenna Gain toward victim (dBi)", value=30.0, step=1.0,
                help="Gain in direction of victim. Nadir (directly below) is worst case.")
            sat_eirp = sat_pt + sat_gt
            st.metric("Single-Sat EIRP", f"{sat_eirp:.1f} dBW = {10**(sat_eirp/10):.0f} W eq.")

            n_vis = st.number_input("Visible Satellites (constellation)", value=15, step=1, min_value=1,
                help="Number of satellites simultaneously visible to victim receiver. "
                     "LEO: 10–30; MEO: 4–12; GEO: 1–3.")
            st.caption(f"Aggregate EIRP = single + 10·log({n_vis}) = **{sat_eirp + 10*np.log10(n_vis):.1f} dBW**")

        with sb:
            st.markdown("**Geometry (P.619)**")
            sat_alt  = st.number_input("Orbital Altitude (km)", value=1200.0, step=100.0,
                help="LEO: 500–2000 km; MEO: 8000–20000 km; GEO: 35786 km.")
            elev_deg = st.number_input("Elevation Angle (°)", value=10.0, step=5.0, min_value=0.0, max_value=90.0,
                help="Elevation angle from horizontal. Worst case for most scenarios = lowest elevation "
                     "consistent with visibility (typically 5–10°).")
            victim_alt = st.number_input("Victim Altitude (km, 0 = ground)", value=0.0, step=0.5,
                help="0 = ground receiver; >0 = airborne receiver. "
                     "Aircraft at 10 km altitude have line-of-sight to more satellites.")

            # Slant range (P.619 geometry)
            import math as _m
            Re = 6371.0  # Earth radius km
            slant_range = _m.sqrt((Re + sat_alt)**2 - (Re*_m.cos(_m.radians(elev_deg + 90)))**2) - Re*_m.sin(_m.radians(elev_deg))
            st.metric("Slant Range", f"{slant_range:.0f} km",
                help="Geometric distance satellite→victim along slant path.")

        # P.619 Path Loss
        # Free-space loss on slant path:
        wpc_freq4c = st.number_input("Frequency (MHz)", value=float(wb['f_low_mhz']), step=10.0, key="wpc_f4c")
        fspl_sat = 20*np.log10(slant_range) + 20*np.log10(wpc_freq4c) + 32.44

        # Atmospheric attenuation at elevation angle (simplified P.676)
        atm_loss = 0.1 / _m.sin(_m.radians(max(elev_deg, 1)))   # ~0.1 dB zenith at L/C band
        total_path_loss = fspl_sat + atm_loss

        # Single-satellite pfd at victim
        d_m = slant_range * 1000
        single_pfd = (sat_eirp - 30) - 10*np.log10(4*np.pi*d_m**2)  # dBW/m²

        # epfd aggregate (sum all visible sats, simplified as n × single)
        epfd = single_pfd + 10*np.log10(n_vis)   # dBW/m²

        # Rx noise temperature and I/N
        st.markdown("**Victim Receiver**")
        rc1, rc2 = st.columns(2)
        with rc1:
            wpc_tsys = st.number_input("System Noise Temperature (K)", value=200.0, step=10.0,
                help="Aircraft terminal system noise temperature. AMS(R)S typical: 150–300 K.")
            wpc_bw4c = st.number_input("Rx Bandwidth (MHz)", value=float(wb['f_high_mhz']-wb['f_low_mhz']),
                step=1.0, key="wpc_bw4c")
        with rc2:
            wpc_rx_g4c = st.number_input("Rx Antenna Gain (dBi)", value=0.0, step=0.5, key="wpc_rxg4c")

        k_boltzmann = 1.38e-23
        noise_power_w = k_boltzmann * wpc_tsys * wpc_bw4c * 1e6
        noise_dbw = 10*np.log10(noise_power_w)

        # Received interference from constellation (using Rx gain and path loss)
        rx_interf_4c = (sat_eirp + 10*np.log10(n_vis)) - total_path_loss + wpc_rx_g4c  # dBW
        i_n_4c = rx_interf_4c - noise_dbw

        # ΔT/T
        delta_t_t = 100 * (10**(i_n_4c/10))   # %

        # Protection check
        i_n_thresh_4c = eff_thresh  # from band selection
        margin_4c = i_n_thresh_4c - i_n_4c

        # epfd limit from band
        epfd_limit = wb.get("epfd_threshold_dbw_m2_mhz", None)
        delta_t_t_limit = wb.get("delta_t_t_pct_single", None)

        st.markdown("---")
        st.subheader("📊 Results (WP 4C)")
        r1,r2,r3,r4,r5 = st.columns(5)
        r1.metric("Slant Path Loss", f"{total_path_loss:.1f} dB")
        r2.metric("Single-Sat PFD", f"{single_pfd:.1f} dBW/m²")
        r3.metric("Constellation epfd", f"{epfd:.1f} dBW/m²",
            help="Aggregate epfd from all visible satellites — this is what S.1586 requires.")
        r4.metric("I/N (aggregate)", f"{i_n_4c:.1f} dB")
        r5.metric("ΔT/T", f"{delta_t_t:.2f} %",
            help="Noise temperature rise. WP 4C criterion: ≤6% single-entry, ≤20% aggregate.")
        ...
        if epfd_limit:
            epfd_margin = epfd_limit - epfd
            ok_epfd = epfd <= epfd_limit
            checks.append(("epfd limit", f"{epfd_limit} dBW/m²/MHz",
                f"{epfd:.1f} dBW/m²", f"{epfd_margin:+.1f} dB", ok_epfd))

        if delta_t_t_limit:
            ok_dt = delta_t_t <= delta_t_t_limit
            checks.append(("ΔT/T (single-entry)", f"≤{delta_t_t_limit}%",
                f"{delta_t_t:.2f}%", f"{delta_t_t_limit - delta_t_t:+.2f}%", ok_dt))

        ok_in = i_n_4c <= i_n_thresh_4c
        checks.append(("I/N threshold", f"{i_n_thresh_4c} dB",
            f"{i_n_4c:.1f} dB", f"{margin_4c:+.1f} dB", ok_in))

        for label, limit, actual, margin_s, ok in checks:
            icon = "✅" if ok else "❌"
            color = "green" if ok else "red"
            st.markdown(f"**{icon} {label}:** limit = {limit} | actual = {actual} | margin = {margin_s}")

        if all(c[4] for c in checks):
            st.success("✅ All WP 4C protection criteria met.")
        else:
            st.error("❌ One or more WP 4C protection criteria violated. "
                     "Cite non-compliance in contribution — request revised P.619-based study.")

        # Key methodological flags
        with st.expander("🔍 Methodology Compliance Checklist (P.619 / S.1586)"):
            st.markdown(f"""
| Requirement | Status |
|---|---|
| Propagation model: P.619 (not P.452) | ✅ P.619 slant-path geometry used |
| Aggregate epfd (not single-sat pfd) | {'✅' if n_vis > 1 else '⚠️'} {n_vis} satellites included |
| Worst-case elevation angle (≤10°) | {'✅' if elev_deg <= 10 else '⚠️ Consider lower elevation'} {elev_deg}° |
| Airborne victim included | {'✅' if victim_alt > 0 else '⚠️ Ground only — add airborne scenario'} |
| ΔT/T metric used for AMS(R)S | {'✅' if delta_t_t_limit else 'N/A for this band'} |
""")

    # ══════════════════════════════════════════════════════════════════════════
    # WP 7B — Lunar SRS (novel geometry)
    # ══════════════════════════════════════════════════════════════════════════
    elif wpc_wp.startswith("WP 7B"):
        st.subheader("🌙 WP 7B Lunar SRS Geometry Check")
        ex("No established ITU-R methodology for Earth-Moon to terrestrial ARNS. "
           "This calculator performs an order-of-magnitude check using free-space path loss "
           "at lunar distances. Use as policy argument tool, not a regulatory submission.")

        la, lb = st.columns(2)
        with la:
            lunar_dist_km = st.number_input("Earth-Moon Distance (km)", value=384400.0, step=1000.0)
            lunar_eirp_dbw = st.number_input("Lunar Transmitter EIRP (dBW)", value=40.0, step=5.0,
                help="40 dBW = 10 kW — large lunar relay. 20 dBW = 100 W — small lander.")
        with lb:
            wpc_freq7b = st.number_input("Frequency (MHz)", value=float(wb['f_low_mhz']), step=10.0, key="wpc_f7b")

        fspl_lunar = 20*np.log10(lunar_dist_km) + 20*np.log10(wpc_freq7b) + 32.44
        d_m_lunar  = lunar_dist_km * 1000
        pfd_lunar  = lunar_eirp_dbw - 10*np.log10(4*np.pi*d_m_lunar**2)  # dBW/m²

        # Max allowed PFD at victim (from RR Appendix 5 for reference)
        noise_floor_lunar = wb['noise_floor_dbm'] - 30  # convert to dBW
        margin_lunar = noise_floor_lunar + abs(eff_thresh) - (pfd_lunar + 10*np.log10((300/wpc_freq7b)**2/(4*np.pi)))

        r1,r2,r3 = st.columns(3)
        r1.metric("Lunar FSPL", f"{fspl_lunar:.1f} dB")
        r2.metric("PFD at Earth Surface", f"{pfd_lunar:.1f} dBW/m²")
        r3.metric("FAA Noise Floor PFD equiv.", f"{noise_floor_lunar + abs(eff_thresh):.0f} dBW/m²",
            help="Approximate PFD level equivalent to the FAA victim threshold")

        headroom = (noise_floor_lunar + abs(eff_thresh)) - pfd_lunar - (
            10*np.log10((300/wpc_freq7b)**2/(4*np.pi)))

        st.info(f"**Geometric headroom: ~{headroom:.0f} dB** below FAA interference threshold. "
                f"At this EIRP level, lunar SRS is not a direct interference threat to {wpc_band_name}. "
                f"The FAA policy argument is **allocation precedent**, not direct interference.")

        st.markdown("""
**FAA Policy Arguments for WP 7B:**
1. **No ITU-R methodology exists** — oppose any CPM text until P.619 equivalent for lunar geometry is developed
2. **Allocation precedent** — primary SRS in ARNS bands weakens future regulatory defense
3. **Large relay stations** — future lunar infrastructure could have much higher EIRP than assumed above
4. **Propose secondary only** — secondary allocation with no coordination rights against ARNS
        """)

    # ══════════════════════════════════════════════════════════════════════════
    # WP 7C — EESS Passive (allocation only)
    # ══════════════════════════════════════════════════════════════════════════
    elif wpc_wp.startswith("WP 7C"):
        st.subheader("🔭 WP 7C EESS Passive — Allocation Impact Analysis")
        ex("EESS passive sensors do not transmit — there is no interference calculation. "
           "The FAA concern is allocation precedent: a co-primary EESS passive entry in an ARNS band "
           "changes the regulatory status and weakens future protection arguments.")

        st.warning("⚠️ No I/N or path loss calculation applies here. EESS passive sensors receive only.")

        st.markdown(f"""
**Impact Assessment for {wpc_band_name} ({wb['f_low_mhz']}–{wb['f_high_mhz']} MHz)**

| Factor | Current Status | After EESS Co-Primary |
|---|---|---|
| RR allocation table | ARNS primary (exclusive) | ARNS + EESS passive co-primary |
| Coordination obligation | None on ARNS | ARNS may need to protect EESS |
| ITU-R precedent | Clean | "Shared" perception weakens defense |
| Link to AI 1.7 (WP 5D) | Strong — exclusive band | Weakened — "already shared" argument |
| ICAO standing | Clear RR 4.10 basis | More complex regulatory picture |

**The indirect regulatory chain:**

```
AI 1.19 (EESS co-primary in RA band)
    ↓
IMT proponents cite "band is shared" in AI 1.7 negotiations
    ↓
OOB emission limits from IMT into RA band are harder to defend
    ↓
FAA Radio Altimeter protection is compromised
```

**US Policy Position:**
- Oppose co-primary status; accept secondary only with no coordination obligation on ARNS
- Cite RR No. 4.10 — harmful interference to safety-of-life services must be prevented
- Request ICAO liaison statement documenting the impact on aviation
- Link to CPM text for AI 1.7 to ensure WPs coordinate their decisions
        """)

        if wb.get('rr_1_59'):
            st.success(f"✅ {wpc_band_name} is a **RR 1.59 safety service** — invoke this in all WP 7C discussions to establish the highest regulatory protection basis.")
